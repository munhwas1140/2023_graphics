...
/// 변환 관련 변수 및 함수
////////////////////////////////////////////////////////////////////////////////
glm::mat4   g_mat_model = glm::mat4(1.f) , g_mat_view, g_mat_proj;
...
void compose_imgui_frame()
{
  ...
  // control window
  {
	...
	float f[3];
	for(int i = 0; i < 3; i++) {
		f[i] = g_vec_model_translate[i];
	}
    ImGui::SliderFloat3("translate", f, -3.0f, 3.0f);
	for(int i = 0; i < 3; i++) {
		g_vec_model_translate[i] = f[i];
	}
	ImGui::gizmo3D("rotation", g_quat_model_rotation);

	float f2[3];
	for(int i = 0; i < 3; i++) {
		f2[i] = g_vec_model_scale[i];
		//f2[i] -= g_mat_model[i][i];
	}
    ImGui::SliderFloat3("sclae", f2, 0.0f, 2.0f);
	for(int i = 0; i < 3; i++) {
		g_vec_model_scale[i] = f2[i];
		//g_vec_model_scale[i] += g_mat_model[i][i];
	}
...
void set_transform() 
{
  ...
  g_mat_model = glm::translate(glm::mat4(1.f), g_vec_model_translate);

//  glm::mat4 T1 = glm::translate(glm::mat4(1.f), glm::vec3(g_mat_model[0][3], g_mat_model[1][3], g_mat_model[2][3]));
//  glm::mat4 T2 = glm::translate(glm::mat4(1.f), glm::vec3(-g_mat_model[0][3], -g_mat_model[1][3], -g_mat_model[2][3]));

  g_mat_model *=  glm::mat4_cast(g_quat_model_rotation);
  g_mat_model *= glm::scale(glm::mat4(1.f), g_vec_model_scale) ;
}
